...
int sc_main(int argc, char *argv[])
{
	...
	std::cout << "[Main] fin de l'allocation de ressources" << std::endl;
	inv1.reset(reset);
	inv1.blank(blank);
	inv1.clk(pckout);
	...
	reset = 1;
	std::cout << "[Main] Lancement de la simulation" << std::endl;
	sc_start(); // Lancement de la simulation
	std::cout << "[Main] Fin de la simulation" << std::endl;
	...
	//system("gtkwave image.vcd &");	// Afficher les chrons
	std::cout << "[Main] Fin du programme" << std::endl;
	exit(0);
}
///////////////////////////////////////////////
